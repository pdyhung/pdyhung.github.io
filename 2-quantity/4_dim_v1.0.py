...
def cad_dim(obs , index_number):
	path1 = acad_Doc.FullName
	name_cad = acad_Doc.Name
	path2 = path1.replace(name_cad , "")
	num = but_click()
	if num == 1:
		with open(path2 + "pdy_dim.csv" , "w" , encoding="utf-8-sig") as file:
			f = 0
			t = ""
			t2 = ""
			for i in obs:
...
def get_value():
	...
	total_length = cad_dim(obs , index_number)
...
bt1 = tk.Button(root, text="Chọn đối tượng dim" , command =lambda:get_value() )
