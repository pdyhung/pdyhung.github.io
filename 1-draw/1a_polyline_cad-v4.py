...
def draw_text_with_polyline(polyline_cad):
	...
	stt = int(input())
	print(polyline_cad.x_list)	
	x = polyline_cad.x_list
	y = polyline_cad.y_list
	for j in range(len(x)):
		a1 = '{0:.{1}f}'.format(float(x[j]),3)
		a2 = '{0:.{1}f}'.format(float(y[j]),3)
		acadModel.AddText(str(j+stt), APoint(float(x[j])+1,float(y[j])+1), 2)


def draw_add_line(polyline_cad):
	print("Số bước là: ")
	delta = int(input())
	x = polyline_cad.x_list
	y = polyline_cad.y_list
	for j in range(0,len(x)-1,delta):
		acadModel.AddLine(APoint(float(x[j]),float(y[j])) ,APoint(float(x[j+1]),float(y[j+1])) )

def draw_add_polyline(polyline_cad):
	print("Số bước là: ")
	delta = int(input())
	x = polyline_cad.x_list
	y = polyline_cad.y_list
	z = polyline_cad.z_list

	for i in range(0,len(x)-1,delta):
		toa_do = []
		toa_do.append(float(x[i]) )
		toa_do.append(float(y[i]) )
		toa_do.append(float(z[i]) )
		toa_do.append(float(x[i+1]) )
		toa_do.append(float(y[i+1]) )
		toa_do.append(float(z[i+1]) )
		b = aDouble(toa_do)
...
def show_menu():
	...
	print("option3: work: ")
	print("option4: draw line: ")
	print("option5: draw polyline: ")
...
def main():
	...
	while True:
		...
		if choice == 1:
			...
			draw_add_line(polyline_cad)
		elif choice == 5:
			draw_add_polyline(polyline_cad)
		# elif choice == 6:
