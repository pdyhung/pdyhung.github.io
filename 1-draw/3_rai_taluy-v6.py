...
def rai_taluy_dap(t, a1, h, cad_pl_2, p_0, cad_pl_1):
	x0 = p_0[0]
	y0 = p_0[1]
	cad_p0 = APoint(x0, y0, 0)
	point_1_list = []  # list point large
		...
		if len(p) == 3:
			...
			for k in range(len(cad_pl_2)):
				try:
					...
					taluy_line_1 = acadModel.AddLine(cad_p0, APoint(p_line[0], p_line[1]))
					taluy_line_2_angle = taluy_line_1.Angle + math.pi
					taluy_line_2_r = (1-n)*taluy_line_1.Length
					taluy_line_2 = acadModel.AddLine(cad_p0, APoint(p_line[0] + taluy_line_2_r*math.cos(taluy_line_2_angle), p_line[1] + taluy_line_2_r*math.sin(taluy_line_2_angle)))
					taluy_line_1.Delete()
				except:
					pass
			cad_p0 = APoint(p[0], p[1], p[2])
...
def show_menu():
	print("......")
	print("WROTE BY PHAM VAN HUNG IN 2022 ___CA DOAN G.X VUNG TAU___ S2")
	print("option 1: taluy dap: ")
	print("option 2: taluy dao: ")
	print("option 3: --> EXIT <-- ")
	print("......")
	...
	while True:
		...
		if choice == 1:
			...
			for j in cad_pl_2_temp:
				cad_pl_2.append(j)
			rai_taluy_dap(t, a1, h, cad_pl_2, p_0, cad_pl_1)
